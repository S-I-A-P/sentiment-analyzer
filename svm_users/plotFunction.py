...
def compass_plot(array):
    ...
    for obj in array:
        ...
        positions.append(plt.text(obj.x_coord, obj.y_coord, obj.username, fontdict=fontLabel))
    ...
    plt.title(label='Political Compass', fontdict=fontTitle)
    plt.xlabel(xlabel='Economic policy', fontdict=fontAxis)
    plt.ylabel(ylabel='Social policy', fontdict=fontAxis)
	
    plt.grid(True)
	
    adjust_text(positions, arrowprops=dict(arrowstyle="->", color='r', lw=0.5), precision=0.01) # force_text=(0.3, 0.5) force_points=(0.5, 0.5)
